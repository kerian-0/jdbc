...
public interface SQLQueries {
	...
	String DISTRICT_TOTAL_COUNT_BY_STATE = "select s.name, count(d.id) district_count from states s join districts d on s.id=d.state_id group by s.id order by district_count desc";
	String RATIO_OF_ONE_DISTRICT_AVGPOPULATION = "select s.name ,s.burmese ,s.region, s.capital,s.population,count(d.id) total_districts ,\r\n"
			+ "ROUND(s.population / count(d.id )) avg_population\r\n"
			+ " from states s left join districts d on s.id=d.state_id group by s.name, s.burmese, s.region, s.capital, s.population  order by avg_population desc";
